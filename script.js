...
// ===== hero: 3D tilt photo card on scroll + mouse (home page only) =====
...
function onHeroScroll(){
  ...
  if(!reduceMotion){
    if(photoCard){
      const rotY = -14 + progress * 26 + mouseX * 10;
      const rotX = 8 - progress * 10 + mouseY * -8;
      const translateY = Math.sin(progress * Math.PI) * -26;
      photoCard.style.transform = `translateY(${translateY}px) perspective(900px) rotateY(${rotY}deg) rotateX(${rotX}deg)`;
    }
    blobs.forEach(b => {
  ...
  if(hc1) hc1.classList.toggle('visible', progress > 0.1 && progress < 0.62);
  if(hc2) hc2.classList.toggle('visible', progress > 0.4 && progress < 0.92);
...
tabs.forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    const target = document.querySelector(`.menu-group[data-group="${tab.dataset.tab}"]`);
